...
let countTotal = () => {
  ...
  let total = 0;
  if (quantity === 2) {total = 300}
  else if (quantity === 3 || quantity === 4) {total = 500}
  else if (quantity === 1) {total = 200}
  else {total = 0};
  $(".choose-title").show();
  $(".choose-message").hide();
...
function getValue() {
    ...
    if (!typesValues.find(checked) && !classesValues.find(checked) && !presentersSelectValues.find(checked)) {
      $('.sessions-table .card, .sessions-table .card a, .more').removeClass('disabled');
    }
...
$('#classes-events input').change(function() {
  $('.sessions-table .card, .sessions-table .card a, #mobile-days .card, #mobile-days .mb-2 a, .more').addClass('disabled');
  getValue();
});
